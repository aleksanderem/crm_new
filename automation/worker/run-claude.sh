...
if [ "$CLAUDE_EXIT" -eq 0 ] && [ "$COMMITS_AHEAD" -gt 0 ]; then
  ...
  if [ -z "$PR_NUM" ] || [ "$PR_NUM" = "null" ]; then
    # Ground Claude's summary in the actual diff: if it cites file:line
    # references and none of those files appear in `git diff origin/main..HEAD`,
    # discard the summary in favour of a fact-only fallback (#1586).
    DIFF_FILES_LIST=$(git diff --name-only origin/main...HEAD)
    GROUND_OUT=$(mktemp)
    if printf "%s" "$CLAUDE_MSG" \
        | DIFF_FILES="$DIFF_FILES_LIST" node /home/claude-bot/worker/ground-pr-body.mjs \
          > "$GROUND_OUT" 2>>"$JOB_LOG"; then
      PR_SUMMARY=$(cat "$GROUND_OUT")
      echo "PR body grounding: OK"
    else
      echo "PR body grounding: REJECTED — citations did not match diff, using safe fallback"
      DIFF_STAT=$(git diff --stat origin/main...HEAD)
      COMMIT_LOG=$(git log --format='- %h %s' origin/main..HEAD)
      PR_SUMMARY="_Claude's narrative summary referenced files that are not in this PR's diff and was discarded ([#1586](https://github.com/${REPO}/issues/1586))._
