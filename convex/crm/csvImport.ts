    ...
    for (let i = 0; i < args.records.length; i++) {
      ...
      try {
        ...
        await db.insert("products", {
          ...
          stockUnit: record.stockUnit?.trim() ?? null,
          minStock: record.minStock ?? null,
          productSection: record.productSection?.trim() ?? null,
          manufacturer: record.manufacturer?.trim() ?? null,
          catalogNumber: record.catalogNumber?.trim() ?? null,
          stockNote: record.stockNote?.trim() ?? null,
          createdBy: String(authResult.userId),
          createdAt: now,
          updatedAt: now,
        });
