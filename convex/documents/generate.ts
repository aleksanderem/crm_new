...
export const generateDocument = action({
  ...
  handler: async (ctx, args) => {
    ...
    // Resolve signer email before generating the token so we can decide whether
    // to set an expiry. When there is no email, omit signingTokenExpiresAt so
    // the hourly expireSigningTokens job does not prematurely expire tokens that
    // were never delivered. Mirrors autoGenerateDocuments.ts:213-224.
    const scopeData = await resolveScopeSupabase(
      db,
      String(args.organizationId),
      args.entityType as EntityType,
    ...
    // Downgrade pending_signature to draft when there is no email address to
    // deliver the signing link to. Mirrors autoGenerateDocuments.ts:204.
    if (status === "pending_signature" && !signerEmail) {
      status = "draft";
    }
