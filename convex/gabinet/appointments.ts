...
import { checkDocumentGate, getAppointmentDocumentCompleteness } from "./_helpers/documentGate";
  ...
  if (args.nextStatus === "cancelled" || args.nextStatus === "no_show") {
    try {
      ...
      for (const doc of linkedDocs) {
        ...
        if (
          docRow.status === "pending_signature" ||
          (docRow.status === "draft" && docRow.autoGenerated === true)
        ) {
...
export const _updateSideEffects = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.treatmentChanged && args.treatmentId && args.patientId) {
      // Void auto-generated draft/pending_signature documents from the previous
      // treatment so they no longer block the document gate (issue #5349).
      // Only docs whose generatedForTreatmentId is known and differs from the
      // new treatment are voided; signed/completed docs remain as history.
      try {
        const voidDb = createSupabaseDb();
        const linkedDocs = await voidDb
          .query("formDocuments")
        ...
        const VOIDABLE_STATUSES = new Set(["draft", "pending_signature"]);
        for (const doc of linkedDocs) {
          const docRow = doc as Record<string, unknown>;
          const genFor = docRow.generatedForTreatmentId as string | null | undefined;
          if (
            docRow.autoGenerated === true &&
            VOIDABLE_STATUSES.has(docRow.status as string) &&
            genFor != null &&
            genFor !== args.treatmentId
          ) {
            await voidDb.patch("formDocuments", docRow._id as string, {
              status: "voided",
              updatedAt: nowMs,
        ...
        console.warn("[_updateSideEffects] void old treatment docs failed (non-fatal):", e);
      }
...
export const updateStatus = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (appt.status === "completed") {
      ...
      if (wonReturnRace) {
        try {
          await ctx.runAction(internal.magazyn.movements.returnStockForAppointment, {
            organizationId: String(args.organizationId),
            appointmentId: args.appointmentId,
        ...
        try {
          ...
          for (const link of links) {
            ...
            try {
              ...
              // FEFO lot tracking (#2992): handled inside consumeForAppointment,
              // which drains tracked lots in expiry order and writes all movements.
              const { negativeStock } = await ctx.runAction(
...
export const getAppointmentDocCompletenessAction = action({
  args: {
    organizationId: v.string(),
    appointmentId: v.string(),
  },
  handler: async (ctx, args) => {
    ...
    return await getAppointmentDocumentCompleteness(
      args.appointmentId as Id<"gabinetAppointments">,
    );
  },
});
    ...
    try {
      await ctx.runMutation(
        internal.gabinet.appointments._cancelSideEffects,
        {
          ...
          reason: args.reason,
          scheduledActivityId: (appt.scheduledActivityId as string) ?? undefined,
          createdBy: appt.createdBy as Id<"users">,
