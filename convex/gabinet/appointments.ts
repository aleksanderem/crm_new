...
export const create = action({
  ...
  handler: async (ctx, args) => {
    ...
    // Triggers when the treatment is either a legacy `treatmentCount > 1`
    // row, or has been migrated to a `packageId` pointing at an
    // auto-generated package (#1533).
    let resolvedPackageUsageId = args.packageUsageId ?? null;
    const treatmentIsLegacyCourse =
      !!treatment && (treatment.treatmentCount ?? 1) > 1;
    const treatmentHasPackageLink = !!treatment && !!treatment.packageId;
    if (
      !resolvedPackageUsageId &&
      treatment &&
      (treatmentIsLegacyCourse || treatmentHasPackageLink)
    ) {
      try {
...
/**
 * Auto-create or attach a package usage record for the patient on first
 * booking. Triggers either when the treatment carries a legacy
 * `treatmentCount > 1` (pre-#1525) or when it has been linked to a real
 * `gabinetTreatmentPackages` row via `packageId` (#1525 / #1533).
 *
 * For legacy rows, we synthesize an auto-generated package keyed by
 * `autoGeneratedForTreatmentId`. For migrated/linked rows, we resolve the
 * package the treatment points at and read its session count from
 * `treatments[].quantity`. Supabase-primary.
 */
async function resolveAutoPackageUsageSupabase(
  ...
  // Prefer an explicit `packageId` link (post-#1525). Fall back to the legacy
  // `treatmentCount`-driven auto-generated package lookup.
  let autoPackage: any = null;
  let treatmentCount: number | null | undefined = null;

  const linkedPackageId = args.treatment.packageId
    ? String(args.treatment.packageId)
    : null;
  if (linkedPackageId) {
    autoPackage = await db.get("gabinetTreatmentPackages", linkedPackageId);
    if (autoPackage) {
      const entries = (autoPackage.treatments as Array<{
        treatmentId: string;
        quantity: number;
      }> | null) ?? [];
      const match = entries.find(
        (e) => String(e.treatmentId) === String(args.treatmentId),
      );
      treatmentCount = match?.quantity ?? null;
    }
  }

  if (!autoPackage) {
    treatmentCount = args.treatment.treatmentCount;
