...
export const purchaseTreatment = action({
  ...
  handler: async (ctx, args): Promise<string> => {
    ...
    if (args.sessionCount < 1) throw new Error("Session count must be at least 1");
    ...
    // Prefer the package the treatment is already linked to (post-#1525),
    // then the legacy auto-generated package keyed on the treatment id, and
    // only create a new auto-generated one as a fallback for un-migrated rows.
    let autoPackage = treatment.packageId
      ? await db.get("gabinetTreatmentPackages", String(treatment.packageId))
      : null;
    if (!autoPackage) {
      autoPackage = await db
        .query("gabinetTreatmentPackages")
        .eq("organizationId", orgIdStr)
    ...
    if (!autoPackage) {
      ...
      const packageId = await db.insert("gabinetTreatmentPackages", {
        ...
        treatments: [{ treatmentId: args.treatmentId, quantity: defaultQuantity }],
    ...
    try {
      await ctx.runMutation(internal.gabinet.packages._purchaseTreatmentSideEffects, {
        usageId,
        organizationId: args.organizationId,
        treatmentId: args.treatmentId,
      ...
      console.error("[packages.purchaseTreatment] Side effects FAILED for usage", usageId, ":", e);
    }
