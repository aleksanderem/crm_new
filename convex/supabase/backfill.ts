    ...
    const { error } = await client.from("gabinet_treatments").upsert(
      {
        ...
        required_form_templates: t.requiredFormTemplates?.map(String) ?? null,
        treatment_parameters: t.treatmentParameters ?? null,
        tag_ids: t.tagIds?.map(String) ?? null,
        requires_completion: t.requiresCompletion ?? null,
...
export const backfillEmployees = internalAction({
  ...
  handler: async (ctx, args): Promise<{ synced: number; errors: string[] }> => {
    ...
    const rows = employees.map((e) => ({
      ...
      role: e.role,
      qualified_treatment_ids: e.qualifiedTreatments?.map(String) ?? [],
...
export const backfillAll = internalAction({
  ...
  handler: async (
    ...
    const userRows = allUsers.map((u: any) => ({
      ...
      image_storage_id: u.imageStorageId ?? null,
      image: u.image ?? null,
      email: u.email ?? null,
