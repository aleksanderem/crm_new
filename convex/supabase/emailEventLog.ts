...
export const writeEmailEventLogToSupabase = internalAction({
  ...
  handler: async (_ctx, args): Promise<{ success: boolean; id: string }> => {
    ...
    if (error) {
      const msg = `Supabase write failed for email_event_log: ${error.message} (code=${error.code})`;
      console.error(msg);
      throw new Error(msg);
    }
...
export const updateEmailEventLogInSupabase = internalAction({
  args: {
    emailEventLogId: v.string(),
    organizationId: v.string(),
  ...
  handler: async (_ctx, args): Promise<{ success: boolean; id: string }> => {
    ...
    if (args.bindingId !== undefined) row.binding_id = args.bindingId;
    ...
    const { data, error } = await client
      .from("email_event_log")
      .update(row)
      .eq("id", args.emailEventLogId)
      .select("id")
      .single();

    if (error) {
      const msg = `Supabase update failed for email_event_log ${args.emailEventLogId}: ${error.message} (code=${error.code})`;
      console.error(msg);
      throw new Error(msg);
    }
    ...
    console.info(`EmailEventLog updated in Supabase id=${data.id} org=${args.organizationId}`);
    return { success: true, id: data.id };
  },
});
