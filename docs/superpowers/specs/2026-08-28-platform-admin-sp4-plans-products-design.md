...
Each write action captures `userId` from `verifyPlatformAdmin` and records an audit entry (action strings `plan_updated` / `product_updated`). **Audit caveat:** `logAudit` writes the org-scoped `audit_log` (Supabase) and requires an `organizationId`; plan/product changes are platform-level, not org-scoped. SP4 audits best-effort with a `"__platform__"` scope sentinel wrapped so a rejected/So-non-fitting audit never blocks the config change (the implementer verifies whether `audit_log.organization_id` has a FK to `organizations`; if it does and rejects the sentinel, SP4 falls back to a `console.info` audit line and proper platform-scoped audit is deferred to SP6). The config write itself must always succeed.
