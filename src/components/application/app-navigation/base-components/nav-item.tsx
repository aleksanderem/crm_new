...
const styles = sortCx({
    root: "group relative flex w-full cursor-pointer items-center rounded-md bg-bg-primary outline-focus-ring transition duration-100 ease-linear select-none hover:bg-bg-primary_hover focus-visible:z-10 focus-visible:outline-2 focus-visible:outline-offset-2",
    rootSelected: "bg-bg-primary_hover hover:bg-bg-secondary_hover",
});
...
export const NavItemBase = ({ current, type, badge, href, icon: Icon, children, truncate = true, onClick }: NavItemBaseProps) => {
    ...
    const labelElement = (
        <span
            className={cx(
                "flex-1 text-md font-semibold text-fg-secondary transition-inherit-all group-hover:text-fg-secondary_hover",
                truncate && "truncate",
                current && "text-fg-secondary_hover",
            )}
        >
            {children}
        </span>
