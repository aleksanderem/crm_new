...
export const CalendarHeader = ({ date, selectedView, onSelectionChange, viewOptions, onClickPrev, onClickNext, onClickToday }: CalendarHeaderProps) => {
    ...
    const renderPeriod = () => {
        ...
        return <span className="text-sm text-muted-foreground">{date.toLocaleString(locale, { weekday: "long" })}</span>;
    };

    return (
        <div className="relative flex flex-col items-start justify-between gap-4 bg-card px-4 py-5 md:flex-row md:px-6">
            ...
            <div className="flex flex-wrap items-center gap-3 gap-y-4 max-md:w-full">
                ...
                <Button iconLeading={Plus} size="md" color="secondary">
                    Add event
                </Button>
            </div>

            <div className="pointer-events-none absolute bottom-0 left-0 w-full border-t border-border"></div>
        </div>
    );
};
