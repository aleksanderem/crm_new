...
import { CalendarSidebar } from "./base-components/calendar-sidebar";
        ...
        <div className={cx("flex flex-1 flex-col", className)}>
            ...
            {targetDateForFooter && (
                <div className="border-t border-border px-4 py-5 md:hidden">
                    <h3 className="text-sm font-semibold text-foreground">{fullDateFormatter.format(targetDateForFooter.toDate(timeZone))}</h3>
                    ...
                    {eventsForFooter.length === 0 && <p className="mt-4 text-xs font-semibold text-muted-foreground">No events for this day.</p>}
                    {eventsForFooter.length > 3 && <p className="mt-4 text-xs font-semibold text-muted-foreground">{`${eventsForFooter.length - 3} more...`}</p>}
                </div>
            )}
        </div>
    );
...
const WeekViewDay = ({ day, isLastDay, visibleEvents, timeZone, slotHeight, setSelectedDate, timeFormatter }: WeekViewDayProps) => {
    ...
    return (
        <div className="flex flex-col border-border">
            <div className="relative flex-1 bg-card" style={{ minHeight: `${48 * slotHeight}px` }}>
                {Array.from({ length: 48 }).map((_, slotIndex) => (
...
const WeekView = ({
    ...
    return (
        <div className={cx("flex flex-1 flex-col overflow-auto", className)}>
            <div className="sticky top-0 z-10 grid grid-cols-7 bg-card pl-18 shadow-sm dark:border-b dark:border-border">
                {days.map((day) => {
            ...
            <div ref={scrollContainerRef} className="relative flex flex-1 overflow-y-auto">
                {/* Time Gutter */}
                <div className="flex h-max w-18 flex-col border-r border-border">
                    {hours.map((hour) => {
...
const DayView = ({
    ...
    return (
        <div className={cx("flex flex-1 flex-col overflow-auto", className)}>
            <div className="sticky top-0 z-20 grid grid-cols-7 bg-card shadow-sm md:hidden dark:border-b dark:border-border">
                {days.map((day) => {
                    const isTodayFlag = isDateToday(day, timeZone);
                    const dayToHighlight =
                        selectedDate && day.compare(currentWeekStart) >= 0 && day.compare(currentWeekEnd) <= 0 ? selectedDate : currentWeekStart;
            ...
            <div className="flex h-full flex-1">
                <div ref={scrollContainerRef} className="relative flex h-full flex-1 overflow-auto">
                    <div className="flex h-max w-14 flex-col border-r border-border md:w-18">
                        {hours.map((hour) => {
...
export const Calendar = ({ events, view: defaultView = "month", className, calendarRef, onSidebarChange, onViewChange }: CalendarProps) => {
    ...
    return (
        <div
            ...
            aria-label="Calendar"
            className={cx(
                "flex flex-col overflow-hidden rounded-xl bg-card shadow-xs ring ring-border",
