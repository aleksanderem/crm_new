...
const Title = (props: ComponentPropsWithRef<"h1">) => {
    ...
    return (
        <h1
            {...props}
            className={cx(
                "text-md font-semibold text-fg-primary",
...
const Description = (props: ComponentPropsWithRef<"p">) => {
    ...
    return <p {...props} className={cx("text-center text-sm text-fg-tertiary", size === "lg" && "text-md", props.className)} />;
};
        ...
        >
            ...
            {avatars.slice(0, avatarSlots.length).map((avatar, i) => {
                ...
                return (
                    <div
                        key={i}
                        className={cx("absolute top-1/2 left-1/2 rounded-full bg-bg-primary p-px shadow-xs ring-[0.5px] ring-black/10", slot.size)}
...
const AvatarGrid = ({ avatars = [], ...props }: AvatarGridProps) => {
    ...
    const renderGridAvatar = (avatar: { src: string; alt?: string }, key: number) => (
        <div key={key} className={cx("shrink-0 bg-bg-primary p-px shadow-xs ring-[0.75px] ring-black/10", config.avatar)}>
            <div
                className={cx(
