...
export const FilterDialog = ({ filters, onApply, onClearAll, onAddFilter, onRemoveFilter, onFilterChange, renderFilterRow, className }: FilterDialogProps) => {
    ...
    return (
        <AriaDialog className={cx("overflow-hidden rounded-lg bg-bg-primary shadow-lg ring-1 ring-border-secondary_alt outline-hidden", className)}>
            {({ close }) =>
                !hasFilters ? (
                    <div className="flex flex-col gap-3 p-4">
                        <div className="flex max-w-[352px] flex-col gap-1 text-sm">
                            <p className="font-semibold text-fg-primary">No filters applied</p>
                            <p className="font-normal text-fg-tertiary">Add filters to narrow down results.</p>
                        </div>
                        <div>
        ...
        <AriaDialogTrigger>
            {children ?? (
                <Button
                    ...
                    className={cx("max-h-9", hasApplied && "bg-bg-primary_hover", className)}
                >
