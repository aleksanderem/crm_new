...
import { Tooltip as AriaTooltip, TooltipTrigger as AriaTooltipTrigger } from "@untitled/base/tooltip/tooltip";
...
export const AvatarAddButton = ({ size, className, title = "Add user", ...props }: AvatarAddButtonProps) => (
    <AriaTooltip title={title}>
        <AriaTooltipTrigger
            ...
            aria-label={title}
            className={cx(
                "flex cursor-pointer items-center justify-center rounded-full border border-dashed border-border-primary bg-bg-primary text-fg-quaternary outline-focus-ring transition duration-100 ease-linear hover:bg-bg-primary_hover hover:text-fg-quaternary_hover focus-visible:outline-2 focus-visible:outline-offset-2 disabled:border-gray-200 disabled:bg-bg-secondary disabled:text-gray-200",
                sizes[size].root,
                className,
            )}
        >
