            ...
            {(state) => (
                <div
                    className={cx(
                        ...
                        !state.isDisabled && "group-hover:bg-bg-primary_hover",
                ...
                >
                    ...
                    <span className={cx("grow truncate text-sm font-semibold text-fg-secondary", state.isFocused && "text-fg-secondary_hover")}>
                        {label || (typeof children === "function" ? children(state) : children)}
                    </span>

                    {addon && <span className="ml-1 shrink-0 pr-1 text-xs font-medium text-fg-quaternary">{addon}</span>}
...
const DropdownPopover = (props: DropdownPopoverProps) => {
    return (
        <AriaPopover
            ...
            className={(state) =>
                cx(
                    "w-62 origin-(--trigger-anchor-point) overflow-auto rounded-lg bg-bg-primary shadow-lg ring-1 ring-border-secondary_alt will-change-transform",
                    state.isEntering &&
