...
const MultiSelectFooter = ({ size = "sm", onReset, onSelectAll, className }: MultiSelectFooterProps) => {
    ...
    return (
        <div className={cx("flex items-center justify-between border-t border-border-secondary p-3", className)}>
            ...
            <div className={cx("flex flex-col gap-1.5", className)}>
                ...
                <AriaDialogTrigger>
                    <AriaButton
                        ...
                        className={(state) =>
                            cx(
                                "relative flex w-full cursor-pointer items-center rounded-lg bg-bg-primary shadow-xs ring-1 ring-border-primary outline-hidden transition duration-100 ease-linear ring-inset",
                        ...
                        >
                            {hasSelection ? (
                                <span className={cx("flex items-center", sizes[size].textContainer)}>
                                    <span className={cx("font-medium text-fg-primary", sizes[size].text)}>
                                        {selectedCountFormatter ? selectedCountFormatter(selectedCount) : `${selectedCount} selected`}
                                    </span>
                                    {supportingText && <span className={cx("text-fg-tertiary", sizes[size].text)}>{supportingText}</span>}
                                </span>
                            ) : (
                    ...
                    <AriaPopover
                        ...
                        className={(state) =>
                            cx(
                                "w-(--trigger-width) origin-(--trigger-anchor-point) overflow-hidden rounded-lg bg-bg-primary shadow-lg ring-1 ring-border-secondary_alt outline-hidden will-change-transform",
                                state.isEntering &&
                    ...
                    >
                        <AriaDialog className="outline-hidden">
                            <AriaAutocomplete filter={contains} inputValue={searchValue} onInputChange={setSearchValue}>
                                {showSearch && (
                                    <div className={cx("border-b border-border-secondary", searchSizes[size].wrapper)}>
                                        <AriaSearchField aria-label="Search" value={searchValue} onChange={setSearchValue} autoFocus>
                                            <div className={cx("flex items-center", searchSizes[size].root)}>
                                                ...
                                                <AriaInput
                                                    placeholder="Search"
                                                    className={cx(
                                                        "w-full appearance-none bg-transparent text-fg-primary caret-alpha-black/90 outline-hidden placeholder:text-placeholder",
