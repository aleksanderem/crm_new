...
import type { IconComponentType } from "@untitled/base/badges/badge-types";
...
import { type SelectItemType, sizes } from "@untitled/base/select/select-shared";
import { TagCloseX } from "@untitled/base/tags/base-components/tag-close-x";
        ...
        <div className="relative flex w-full min-w-0 flex-1 flex-row flex-wrap items-center justify-start gap-1.5">
            {!isSelectionEmpty &&
                tagSelectContext?.selectedItems?.items?.map((value) => (
                    <span
                        key={value.id}
                        className={cx(
                            "flex min-w-0 items-center rounded-md bg-bg-primary ring-1 ring-border-primary ring-inset",
                    ...
                    >
                        ...
                        <p
                            className={cx(
                                "truncate font-medium whitespace-nowrap text-fg-secondary select-none",
            ...
            <div className={cx("relative flex min-w-12 flex-1 flex-row items-center", !isSelectionEmpty && "ml-0.5", shortcut && "min-w-[30%]")}>
                <AriaInput
                    ...
                    className={cx(
                        "w-full flex-[1_0_0] appearance-none bg-transparent text-ellipsis text-fg-primary caret-alpha-black/90 outline-hidden placeholder:text-placeholder focus:outline-hidden disabled:cursor-not-allowed",
                        sizes[size].text,
                    )}
                />
