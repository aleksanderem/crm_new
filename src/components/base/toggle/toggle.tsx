...
export const ToggleBase = ({ className, isHovered, isDisabled, isFocusVisible, isSelected, slim, size = "sm" }: ToggleBaseProps) => {
    ...
    return (
        <div
            className={cx(
                "cursor-pointer rounded-full bg-bg-tertiary ring-[0.5px] ring-border-secondary outline-focus-ring transition duration-150 ease-linear ring-inset",
                ...
                <>
                    ...
                    {(label || hint) && (
                        <div className={cx("flex flex-col", styles[size].textWrapper)}>
                            {label && <p className={cx("text-fg-secondary select-none", styles[size].label)}>{label}</p>}
                            {hint && (
