...
function reasonLabel(reason: StockMovementReason, t: ReturnType<typeof useTranslation>["t"]): string {
  const map: Record<StockMovementReason, string> = {
    ...
    reserved: t("products.stock.reason.reserved", { defaultValue: "Rezerwacja" }),
    reservation_release: t("products.stock.reason.reservation_release", { defaultValue: "Zwolnienie rezerwacji" }),
    direct_sale: t("products.stock.reason.direct_sale", { defaultValue: "Sprzedaż bezpośrednia" }),
    direct_sale_return: t("products.stock.reason.direct_sale_return", { defaultValue: "Zwrot ze sprzedaży bezpośredniej" }),
