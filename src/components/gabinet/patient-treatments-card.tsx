...
export function PatientTreatmentsCard({ patientId, organizationId }: PatientTreatmentsCardProps) {
  ...
  // Only show usages that were created via the treatment-sale flow
  // (i.e. their package is auto-generated for a single treatment).
  const items = (usages ?? []).filter((usage) => {
    const pkg = packageMap.get(usage.packageId);
    return Boolean(pkg?.autoGeneratedForTreatmentId);
  });
