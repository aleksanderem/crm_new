...
describe("appointment state machine", () => {
  ...
  test("cancelled -> scheduled restores voided formDocuments to pending_signature", async () => {
    const t = createManagedTestCtx();
    const { organizationId, userId, identity } = await seedTestUser(t);
    const { patientId, treatmentId } = await seedGabinetPrereqs(t, organizationId, userId);
    ...
    // Seed a voided formDocument simulating what happens when an appointment
    // is cancelled (pending_signature docs get voided by _cancelSideEffects).
    const db = createSupabaseDb();
    const now = Date.now();
    const voidedDocId = await db.insert("formDocuments", {
      organizationId: String(organizationId),
      templateId: "template-test-id",
      title: "Consent Form",
      ...
      timing: "before_start",
      autoGenerated: true,
      signingToken: "old-expired-token",
      signingTokenExpiresAt: now - 1000,
    ...
    // The voided doc must be restored to pending_signature with a fresh token.
    const restoredDoc = await db.get<{
      status: string;
      signingToken: string;
      signingTokenExpiresAt: number | null;
    }>("formDocuments", voidedDocId);
    expect(restoredDoc?.status).toBe("pending_signature");
    expect(restoredDoc?.signingToken).not.toBe("old-expired-token");
    expect(restoredDoc?.signingTokenExpiresAt).toBeNull();
  });

  test("no_show -> scheduled restores voided formDocuments to pending_signature", async () => {
    const t = createManagedTestCtx();
    const { organizationId, userId, identity } = await seedTestUser(t);
    const { patientId, treatmentId } = await seedGabinetPrereqs(t, organizationId, userId);
    ...
    const voidedDocId = await db.insert("formDocuments", {
      ...
      timing: "before_start",
      autoGenerated: true,
      signingToken: "old-expired-token-2",
      signingTokenExpiresAt: now - 1000,
      responseData: JSON.stringify({}),
      createdBy: String(userId),
      createdAt: now,
    ...
    expect(restoredDoc?.status).toBe("pending_signature");
    expect(restoredDoc?.signingToken).not.toBe("old-expired-token-2");
    expect(restoredDoc?.signingTokenExpiresAt).toBeNull();
  });
